...
export function buildEvidenceSummary(data: SearchResponse) {
    ...
    if (data.summary.maximumObservedWindGustMph !== undefined) {
        ...
        pieces.push(`Event-day observations include a maximum gust of ${formatMeasurement(data.summary.maximumObservedWindGustMph, "mph", 1)}${observedWind ? ` from ${evidenceSourceLabel(observedWind)}` : ""}${proximity ? `, ${proximity} from the property` : ""}.`);
    }
    if (data.summary.maximumReportedWindGustMph !== undefined) {
        const proximity = recordProximity(reportedWind);
        pieces.push(`Nearby storm reports include a maximum reported gust of ${formatMeasurement(data.summary.maximumReportedWindGustMph, "mph", 1)}${proximity ? `, ${proximity} from the property` : ""}.`);
    }
    if (data.summary.maximumReportedHailInches !== undefined) {
        pieces.push(`Nearby Local Storm Reports include hail up to ${formatMeasurement(data.summary.maximumReportedHailInches, "in")} within the searched radius.`);
    }
    if (data.summary.localStormReportCount > 0) {
        pieces.push(`${data.summary.localStormReportCount} nearby Local Storm Report${data.summary.localStormReportCount === 1 ? " was" : "s were"} returned.`);
    }
    if (data.summary.warningCount > 0) {
        pieces.push(`${data.summary.warningCount} archived warning polygon${data.summary.warningCount === 1 ? "" : "s"} intersected the property point.`);
    }
    if (pieces.length === 0) {
        pieces.push("The searched sources returned no substantive wind, hail, warning, or storm-report records for this location and window.");
    }
    pieces.push("These records describe available evidence near the property and do not establish damage, causation, coverage, or claim outcome.");
    return pieces.join(" ");
}
