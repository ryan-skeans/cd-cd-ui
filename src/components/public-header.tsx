...
export default function PublicHeader({ homeowner = false, seamlessAtTop = false }: { homeowner?: boolean; seamlessAtTop?: boolean }) {
    ...
    const headerClassName = seamlessAtTop
        ? `fixed inset-x-0 top-0 z-50 transition-[background-color,border-color] duration-200 ${isAtTop ? "border-b border-transparent bg-transparent" : "border-b border-brand-gray/80 bg-white"}`
        : "sticky top-0 z-50 border-b border-brand-gray/80 bg-white";

    return (
        <>
            {seamlessAtTop && <span ref={topSentinelRef} className="pointer-events-none absolute left-0 top-0 h-px w-px" aria-hidden="true" />}
